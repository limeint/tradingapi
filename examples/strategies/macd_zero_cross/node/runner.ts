  ...
  const state = await api.orders.placeOrder(
    {
      ...
      clientOrderId: `macd0-${signal === "entry" ? "b" : "s"}-${seconds}`,
...
const processBar = async (
  ...
  log(
    config,
    "INFO",
    `Closed bar: close=${closes.at(-1)} macd=${format(result.macd)} ` +
      `macd_signal=${format(result.signalLine)} histogram=${format(result.histogram)} ` +
...
export const run = async (api: StrategyApi, config: Config): Promise<void> => {
  ...
  log(
    config,
    "INFO",
    `History ready: close=${state.closes.at(-1)} macd=${format(result.macd)} ` +
      `macd_signal=${format(result.signalLine)}`,
  );

  if (config.check) {
    console.log(
      `History check passed: close=${state.closes.at(-1)} macd=${format(result.macd)} ` +
        `macd_signal=${format(result.signalLine)} histogram=${format(result.histogram)} ` +
        `signal=${result.signal ?? "none"}`,
    );
    return;
