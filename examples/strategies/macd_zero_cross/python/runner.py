"""Market-data and order orchestration for the MACD zero-cross example."""
...
def _format(value: Decimal) -> str:
    """Round a MACD value for display only; the rule uses the full value."""
...
def place_order(
    ...
    state = client.orders.PlaceOrder(
        Order(
            ...
            client_order_id=f"macd0-{'b' if signal == 'entry' else 's'}-{suffix}",
            comment="MACD zero cross",
        )
    )
...
def run(client: TradeAPIClient, config: Config) -> None:
    ...
    logger.info(
        "History ready: close=%s macd=%s macd_signal=%s",
        closes[-1],
        _format(result.macd),
        _format(result.signal_line),
    )

    if config.check:
        print(
            f"History check passed: close={closes[-1]} macd={_format(result.macd)} "
            f"macd_signal={_format(result.signal_line)} "
            f"histogram={_format(result.histogram)} signal={result.signal or 'none'}"
        )
        return
    ...
    for response in client.market_data.SubscribeBars(request):
        for bar in ordered_bars(response.bars):
            ...
            logger.info(
                "Closed bar: close=%s macd=%s macd_signal=%s histogram=%s signal=%s",
                closes[-1],
                _format(result.macd),
                _format(result.signal_line),
                _format(result.histogram),
