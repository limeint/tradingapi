...
import com.yourbrand.tradeapi.*

object SubscribeBarsWithoutAuthorizationHeader : WsSubscriptionBaseExample() {
    private val logger: Logger = LoggerFactory.getLogger(SubscribeBarsWithoutAuthorizationHeader::class.java)

    @JvmStatic
    fun main(args: Array<String>) {
        val symbol = "SBERF@RTSX"
        val subscribeRequest = WsRequest.subscribeBarsRequest(symbol, TimeFrame.TIME_FRAME_D)
        logger.info("Subscribe bars {} without authorization header", symbol)

        run(subscribeRequest = subscribeRequest, withAuthorizationHeader = false) { message ->
